...
oyunkeyf.StrongSocket = function(url, version, settings) {
  ...
  var nbConnects = 0;
  var storage = oyunkeyf.storage.make('surl6');
  ...
  var connect = function() {
    ...
    try {
      ...
      ws.onopen = function() {
        ...
        pingNow();
        // oyunkeyf.pubsub.emit('socket.open')();
        ackable.resend();
      };
  ...
  var pong = function() {
    ...
    // oyunkeyf.pubsub.emit('socket.lag')(averageLag);
  };
  ...
  var onSuccess = function() {
    ...
    if (nbConnects == 1) {
      ...
      // oyunkeyf.idleTimer(10 * 60 * 1000, function() {
      //   options.idle = true;
      //   disconnectTimeout = setTimeout(oyunkeyf.socket.destroy, 2 * 60 * 60 * 1000);
      // }, function() {
      //   options.idle = false;
...
oyunkeyf.StrongSocket.sri = Math.random().toString(36).slice(2, 12);
