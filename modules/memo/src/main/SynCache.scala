...
final class Syncache[K, V](
  ...
  def sync(k: K): V = {
    ...
    if (v != null) v
      ...
      //     if (oyun.common.PlayApp startedSinceSeconds uptime) default(k) // waitForResult(k, fu, duration)
      //     else default(k)
      // }
    }
  }
  ...
  private val loadFunction = new java.util.function.Function[K, Fu[V]] {
    def apply(k: K) = compute(k).withTimeout(
      ...
      // error = oyun.base.OyunException(s"Syncache $name $k timed out after $resultTimeout")
      error = new Exception(s"Syncache $name $k timed out after $resultTimeout")
    )
  ...
  //     // oyun.mon.measureIncMicros(_ => incWaitMicros)(fu await duration)
  //   } catch {
